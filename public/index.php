...
$configuredStorage = getenv('LOGBUCH_STORAGE_PATH');
...
if (is_file($earlyStoragePath . '/updates/maintenance.json')) {
    ...
    if (str_starts_with($earlyRequestPath, '/api/')) {
        ...
        echo json_encode(['error' => 'Das Logbuch wird gerade aktualisiert. Bitte gleich erneut versuchen.'], JSON_UNESCAPED_UNICODE);
    } else {
        header('Content-Type: text/html; charset=utf-8');
        echo '<!doctype html><html lang="de"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Das Logbuch wird aktualisiert</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#f6f5f2;color:#25282d;font:16px system-ui}main{max-width:32rem;padding:2rem;text-align:center}h1{font-size:1.6rem}p{color:#69707a;line-height:1.6}</style><main><h1>Das Logbuch wird aktualisiert</h1><p>Die neue Version wird gerade sicher eingespielt. Diese Seite kann in wenigen Augenblicken neu geladen werden.</p></main></html>';
    }
    exit;
}
...
use Logbuch\Application;
use Logbuch\HttpError;
...
try {
    ...
    echo json_encode(['error' => 'Das Logbuch konnte die Anfrage nicht verarbeiten.'], JSON_UNESCAPED_UNICODE);
}
