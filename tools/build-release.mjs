...
if (!/^ghcr\.io\/[a-z0-9._/-]+$/.test(updaterImage || '')) throw new Error('Updater-Image ist ungültig.');
if (!/^sha256:[a-f0-9]{64}$/.test(updaterDigest || '')) throw new Error('Updater-Digest ist ungültig.');

const root = resolve(new URL('..', import.meta.url).pathname);
const outputPath = resolve(root, output);
const stage = join(outputPath, `logbuch-web-${version}`);
